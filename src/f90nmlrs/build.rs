...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let version_suffix = if Path::new(gitrepo_path).exists() {
        ...
        let workspace_dirty = check_git_dirty_excluding_current()
            .unwrap_or("");
        ...
        let local_dirty = check_git_dirty_current()
            .unwrap_or("");
    ...
    println!("cargo:rustc-env=F90NMLRS_VERSION={}", full_version);
