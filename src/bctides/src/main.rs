...
use schismrs_bctides::tides::HamtideSourceType;
                    ...
                    match matches.get_one::<String>(&format!("elevation-{}-tidal-db", fort_id)) {
                        Some(tidal_db) => match get_tidal_db_possible_values_map().get(tidal_db) {
                            ...
                            Some(TidalDbConfigType::HAMTIDE) => tides::TidalDatabase::HAMTIDE {
                                source: tides::HamtideSourceType::FromDefaultAPI,
                            },
                ...
                let database =
                    match matches.get_one::<String>(&format!("velocity-{}-tidal-db", fort_id)) {
                        Some(tidal_db) => match get_tidal_db_possible_values_map().get(tidal_db) {
                            ...
                            Some(TidalDbConfigType::FES) => tides::TidalDatabase::FES,
                            Some(TidalDbConfigType::HAMTIDE) => tides::TidalDatabase::HAMTIDE {
                                source: HamtideSourceType::FromDefaultAPI,
                            },
                            None => panic!("Unreachable!"),
                        },
                        None => panic!("Unreachable"),
...
fn entrypoint() -> Result<(), Box<dyn std::error::Error>> {
    ...
    macro_rules! config_match {
        ($config:ident) => {
            match &cli.boundary_config.$config {
                Some(cfg) => {
                    builder.$config(cfg);
                }
                None => {}
    ...
    config_match!(elevation);
    config_match!(velocity);
    config_match!(temperature);
    config_match!(salinity);
    let bctides = builder.build()?;
    println!("{}", &bctides);
    Ok(())
}
